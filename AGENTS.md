...
1. Start from [`skill/SKILL.md`](skill/SKILL.md): skill index, pipeline, state machine, directory layout, and `state.md` semantics (`status` vs `current_stage`). Per-run scope extras live in `.workflow/runs/<run-id>/run-brief.md` (optional; see [`skill/run-init/SKILL.md`](skill/run-init/SKILL.md)). To resume or inspect progress without advancing the pipeline, use [`skill/run-status/SKILL.md`](skill/run-status/SKILL.md).
